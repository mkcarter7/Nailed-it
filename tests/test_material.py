...
class MaterialViewSetTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('materials-list')
    ...
    def test_get_material(self):
        response = self.client.get(reverse('materials-detail', args=[self.material.id]))
    ...
    def test_update_material(self):
        ...
        response = self.client.put(reverse('materials-detail', args=[self.material.id]), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], 'Updated Wood')

    def test_delete_material(self):
        response = self.client.delete(reverse('materials-detail', args=[self.material.id]))
