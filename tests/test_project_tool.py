...
class ProjectToolViewSetTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('projecttool-list')
    ...
    def test_create_project_tool(self):
        ...
        self.assertEqual(response.data['tool'], self.tool_1.id)

    def test_get_project_tools(self):
        response = self.client.get(reverse('projecttool-list'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)  # Two tools should be linked to the project

    def test_get_project_tool(self):
        response = self.client.get(reverse('projecttool-detail', args=[self.project_tool_1.id]))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['tool'], self.tool_1.id)
        self.assertEqual(response.data['project'], self.project.id)
    ...
    def test_update_project_tool(self):
        data = {
            'project': self.project.id,
            'tool': self.tool_2.id,  # Update to link the project to a different tool
        }
        response = self.client.put(reverse('projecttool-detail', args=[self.project_tool_1.id]), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['tool'], self.tool_2.id)

    def test_delete_project_tool(self):
        response = self.client.delete(reverse('projecttool-detail', args=[self.project_tool_1.id]))
