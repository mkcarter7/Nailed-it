...
class RoomViewSetTest(APITestCase):
    ...
    def test_get_room(self):
        response = self.client.get(reverse('rooms-detail', args=[self.room.id]))
    ...
    def test_update_room(self):
        ...
        response = self.client.put(reverse('rooms-detail', args=[self.room.id]), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], 'Updated Living Room')

    def test_delete_room(self):
        response = self.client.delete(reverse('rooms-detail', args=[self.room.id]))
